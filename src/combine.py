...
def test(fileDelay, fileEsxtop, fileFailure, fileEsxtopAndDelay):
    ...
    time_delay = str(
        datetime.datetime.fromtimestamp(data_delay["0"][0])
        - datetime.timedelta(0, 28800, 0)
    )[11:].replace(":", "")
...
def time_change(time):
    ...
    min = int(time[2:4])
    sec = int(time[4:6])
    time_new = datetime.timedelta(
        0, hour * 3600 + min * 60 + sec - 8 * 3600 + 120 + 15, 0
    )
...
def esxtop():
    ...
    arr = [
        r"(PDH-CSV 4.0) (UTC)(0)",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% CPU Latency",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% Demand",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% Idle",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% Overlap",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% Ready",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% Run",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% Used",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% VmWait",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\% Wait",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\Migrates/sec",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\Power Usage Watts",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\Switches/sec",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\Timers/sec",
        r"\\localhost\Group Cpu(2012201:FlowMonitor-csy)\Wakeups/sec",
...
def extop_dpvs():
    ...
    arr = [
        r"(PDH-CSV 4.0) (",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\% CPU Latency",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\% Demand",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\% Idle",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\% Overlap",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\% Ready",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\% Run",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\% Used",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\% Wait",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\Power Usage Watts",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\Switches/sec",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\Timers/sec",
        r"\\localhost\Group Cpu(37093751:dpvs_csy)\Wakeups/sec",
        r"\\localhost\Network Port(vnf_switch:67108883:4441444:dpvs_csy)\MBits Received/sec",
        r"\\localhost\Network Port(vnf_switch:67108884:4441444:dpvs_csy)\MBits Transmitted/sec",
        r"\\localhost\Virtual Disk(dpvs_csy)\Commands/sec",
        r"\\localhost\Virtual Disk(dpvs_csy)\Reads/sec",
        r"\\localhost\Virtual Disk(dpvs_csy)\Writes/sec",
    ]
...
def insertDelay(fileDelay, fileEsxtop, fileName):
    ...
    for i in range(len(data_esxtop)):
        ...
        for j in range(len(data_delay)):
            time_delay = str(
                datetime.datetime.fromtimestamp(data_delay["0"][j])
                - datetime.timedelta(0, 28800, 0)
            )[11:].replace(":", "")
...
def insertFailure(fileFailure, fileEsxtopAndDelay, fileName):
    ...
    for i in range(len(data_esxtopanddelay)):
        ...
        for j in range(len(data_failure)):
            time_failure = time_change(
                data_failure["timestamp"][j][18:26].replace(":", "")
            )
...
def insertFailure_dpvs():
    ...
    for i in range(len(data_esxtop)):
        ...
        for j in range(len(cpu_failure)):
            time_failure = time_change(
                cpu_failure["timestamp"][j][18:26].replace(":", "")
            )
        ...
        for j in range(len(vm_failure)):
            time_failure = time_change(
                vm_failure["timestamp"][j][18:26].replace(":", "")
            )
            if time_esxtop == time_failure:
                for k in range(18):
                    point.append(data_esxtop[n[k]][i])
        ...
        for j in range(len(io_failure)):
            time_failure = time_change(
                io_failure["timestamp"][j][18:26].replace(":", "")
            )
            if time_esxtop == time_failure:
                for k in range(18):
                    point.append(data_esxtop[n[k]][i])
